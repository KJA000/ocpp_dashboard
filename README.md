...
충전기(SECC) – 서버(CSMS) – 사용자 인터페이스(Frontend) 간의 통신 및 상태 흐름을
시뮬레이션하고 시각화하는 전기차 충전 관리 시스템입니다.

Frontend 대시보드를 통해 충전 상태를 확인하고,
Backend 서버와 가상 충전기(SECC)를 연동하여 인증 → 플러그 연결 → 충전 시작의
전체 시나리오를 재현합니다.
...
충전 상태 및 이벤트 흐름을 시각적으로 확인할 수 있습니다.
...
1. SECC GUI 실행 후 CSMS 연결

2. GUI에서 Id valid 상태 확인

3. Insert Plug 버튼 선택

4. 충전 상태를 Charging 으로 변경

5. Frontend 대시보드에서 상태 변화 확인

위 과정을 통해 인증 → 연결 → 충전 시작의 OCPP 흐름을 확인할 수 있습니다.
